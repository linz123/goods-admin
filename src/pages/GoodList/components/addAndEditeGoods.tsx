...
const AddAndEditeGoods: React.FC<API.addAndEditFormProps> = ({ type, currentInfo, onClose }) => {
    ...
    const { goodId, title, goodsName, price, tipId, classId, labelId, thumbId, imgId } = currentInfo
    ...
    const onSeletChange = (type: string, val: string | number) => {
        ...
        type === 'tips' ? setTip(val) : type === 'class' ? setClasses(val) : type === 'label' ? setLabels(val) : null
    }
    ...
    return (
        <>
            <h3 className={styles.title}>
                {type === 'add' ? <FormattedMessage id="pages.goodlist.add" defaultMessage="add" /> : <FormattedMessage id="pages.goodlist.eidt" defaultMessage="eidt" />}
            </h3>
            <Form
                name="basic"
                labelCol={{ span: 4 }}
            ...
            >
                <Form.Item
                    ...
                    rules={[{ required: true, message: intl.formatMessage({ id: 'pages.goodlist.inputTitle', defaultMessage: 'inputTitle' }) }]}
                >
                    <Input />
                </Form.Item>
                <Form.Item
                    name="goodsName"
                    label={intl.formatMessage({ id: 'pages.goodlist.goodsName', defaultMessage: 'goodsName', })}
                    rules={[{ required: true, message: intl.formatMessage({ id: 'pages.classlist.inputName', defaultMessage: 'inputName' }) }]}
                >
                    <Input />
                </Form.Item>
                <Form.Item
                    ...
                    rules={[{ required: true, message: intl.formatMessage({ id: 'pages.tipslist.inputMount', defaultMessage: 'inputMount' }) }]}
                >
                ...
                >
                    <SelectList type="tips" nameStr="priceName" idStr="tipId" defaultPropsVal={tipId} onChange={(type: string, val: number) => onSeletChange(type, val)} />
                </Form.Item>
                <Form.Item
                    name="classId"
                    label={intl.formatMessage({ id: 'pages.goodlist.class', defaultMessage: 'class', })}
                >
                    <SelectList type="class" nameStr="className" idStr="classId" defaultPropsVal={classId} onChange={(type: string, val: string) => onSeletChange(type, val)} />
                </Form.Item>
                <Form.Item
                    name="labelId"
                    label={intl.formatMessage({ id: 'pages.goodlist.label', defaultMessage: 'label', })}
                >
                    <SelectList type="label" nameStr="labelName" idStr="labelId" defaultPropsVal={labelId} onChange={(type: string, val: string) => onSeletChange(type, val)} />
                </Form.Item>
                <Form.Item
                    name="thumbId"
                    label={intl.formatMessage({ id: 'pages.goodlist.thumbId', defaultMessage: 'thumbId', })}
                >
                    <UploadCommon type="thumbId" defaultPropsVal={thumbId} onChange={(type: string, val: string) => onUploadChange(type, val)} />
                </Form.Item>
                <Form.Item
                    name="imgId"
                    label={intl.formatMessage({ id: 'pages.goodlist.imgId', defaultMessage: 'imgId', })}
                >
                    <UploadCommon type="imgId" defaultPropsVal={imgId} onChange={(type: string, val: string) => onUploadChange(type, val)} />
                </Form.Item>
                <Form.Item
                    name="describe"
                ...
                <Form.Item wrapperCol={{ offset: 8, span: 16 }}>
                    <Button
                        type="primary"
                        htmlType="submit"
                    ><FormattedMessage id="pages.goodlist.sub" defaultMessage="submit" /></Button>
                </Form.Item>
            </Form>
        </>
    )
